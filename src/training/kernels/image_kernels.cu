...
#pragma unroll
    ...
    // Difference-of-Gaussians residual on luminance. Separable 1D passes,
    // sigma=1 (radius 3) and sigma=2 (radius 6), replicate borders.
    constexpr int kDogRadius1 = 3;
    constexpr int kDogRadius2 = 6;

    template <typename T>
    __device__ __forceinline__ float dog_luma_sample(
        ...
        if (chw) {
            ...
            r = canny_input_value(img, idx);
            g = canny_input_value(img, idx + hw);
            b = canny_input_value(img, idx + 2 * hw);
        } else {
            const int idx = (y * width + x) * 3;
            r = canny_input_value(img, idx);
            g = canny_input_value(img, idx + 1);
            b = canny_input_value(img, idx + 2);
        }
        return 0.299f * r + 0.587f * g + 0.114f * b;
    }

    template <typename T>
    __global__ void dog_horizontal_kernel(
        const T* __restrict__ img,
        float* __restrict__ g1_h,
        float* __restrict__ g2_h,
        int height,
        int width,
        bool chw) {
        ...
        float w2 = 0.0f;
#pragma unroll
        for (int k = -kDogRadius2; k <= kDogRadius2; ++k) {
            const float lum = dog_luma_sample(img, y, x + k, height, width, chw);
            const float k2 = static_cast<float>(k * k);
            const float gw2 = expf(-0.5f * k2 * 0.25f); // 1/(2*sigma^2) with sigma=2
            acc2 += gw2 * lum;
            w2 += gw2;
            if (k >= -kDogRadius1 && k <= kDogRadius1) {
                const float gw1 = expf(-0.5f * k2); // sigma=1
                acc1 += gw1 * lum;
                w1 += gw1;
            }
        }
        const int idx = y * width + x;
        g1_h[idx] = acc1 / fmaxf(w1, 1e-12f);
        g2_h[idx] = acc2 / fmaxf(w2, 1e-12f);
    }

    __global__ void dog_vertical_kernel(
        const float* __restrict__ g1_h,
        const float* __restrict__ g2_h,
        float* __restrict__ dog_out,
        const float* __restrict__ other_dog,
        float* __restrict__ error_hw,
        float weight,
        int height,
        int width) {
        const int x = static_cast<int>(blockIdx.x * blockDim.x + threadIdx.x);
...
#pragma unroll
        for (int k = -kDogRadius2; k <= kDogRadius2; ++k) {
            const int yy = min(max(y + k, 0), height - 1);
            const int idx = yy * width + x;
            const float k2 = static_cast<float>(k * k);
            const float gw2 = expf(-0.5f * k2 * 0.25f);
            acc2 += gw2 * g2_h[idx];
            w2 += gw2;
            if (k >= -kDogRadius1 && k <= kDogRadius1) {
                const float gw1 = expf(-0.5f * k2);
                acc1 += gw1 * g1_h[idx];
                w1 += gw1;
            }
        }
        const float dog = fabsf(acc1 / fmaxf(w1, 1e-12f) - acc2 / fmaxf(w2, 1e-12f));
        const int out_idx = y * width + x;
        if (error_hw != nullptr && other_dog != nullptr) {
            error_hw[out_idx] += weight * fabsf(other_dog[out_idx] - dog);
        } else {
            dog_out[out_idx] = dog;
        }
    }
    ...
    void launch_dog_horizontal(
        ...
        dog_horizontal_kernel<T><<<grid, block, 0, stream>>>(img, g1_h, g2_h, height, width, chw);
        LFS_CUDA_LAUNCH_CHECK(stream, "training.image.dog_horizontal");
    }
    ...
    void launch_add_dog_residual(
        ...
        cudaStream_t stream) {
        stream = resolve_stream(stream);
        if (error_hw == nullptr || scratch_3hw == nullptr || render_f32 == nullptr ||
            height <= 0 || width <= 0 || !(weight > 0.0f) ||
            (gt_f32 == nullptr && gt_u8 == nullptr)) {
            return;
        }

        const int hw = height * width;
        float* g1_h = scratch_3hw;
        float* g2_h = scratch_3hw + hw;
        float* dog_gt = scratch_3hw + 2 * hw;

        if (gt_u8 != nullptr) {
            launch_dog_horizontal(gt_u8, g1_h, g2_h, height, width, gt_chw, stream);
        } else {
            launch_dog_horizontal(gt_f32, g1_h, g2_h, height, width, gt_chw, stream);
        }

        const dim3 block(16, 16);
        const dim3 grid((width + 15) / 16, (height + 15) / 16);
        dog_vertical_kernel<<<grid, block, 0, stream>>>(
            g1_h, g2_h, dog_gt, nullptr, nullptr, 0.0f, height, width);
        LFS_CUDA_LAUNCH_CHECK(stream, "training.image.dog_vertical_gt");

        launch_dog_horizontal(render_f32, g1_h, g2_h, height, width, render_chw, stream);
        dog_vertical_kernel<<<grid, block, 0, stream>>>(
            g1_h, g2_h, nullptr, dog_gt, error_hw, weight, height, width);
        LFS_CUDA_LAUNCH_CHECK(stream, "training.image.dog_vertical_add");
    }
} // namespace lfs::training::kernels
